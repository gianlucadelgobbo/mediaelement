package  
{
	...
	public class FlashMediaElement extends MovieClip {
		...
		public function FlashMediaElement() {
			...
			// debug parameters
			var paramsDebug:Object = {
				id:"me_flash_0", // Non usato
				isvideo:true,
				autoplay:true,
				autohide:false,
				preload:"none",
				...
				startvolume:0.8,
				timerrate:250,
				//flashstreamer:, // Non usato
				//height:288, // Non usato
				file:"rtmp://www.raw-news.net/oflaDemo/testfullscreen",
				//file:"http://video.ted.com/talks/podcast/AlGore_2006_480.mp4",
				//file:"rtmp://stream2.france24.yacast.net/france24_live/en/f24_liveen",
				debug:true,
				controls:true
			}
			//params = paramsDebug;
			...
			_stageHeight = stage.stageHeight - 30;
			...
			if (ExternalInterface.available) { //  && !_alwaysShowControls
				...
				try {
					if (ExternalInterface.objectID != null && ExternalInterface.objectID.toString() != "") {
						...
						ExternalInterface.addCallback("mouseHide", mouseHide);
						ExternalInterface.addCallback("mouseShow", mouseShow);
		...
		*/
		...
		function resizeHandler(e:Event):void {
			_output.appendText("resizeHandler: " + _isFullScreen + "\n"); 
			_stageWidth = stage.stageWidth;
			_stageHeight = stage.stageHeight - (_isFullScreen ? 0 : 30);
		...
		function stageFullScreenChanged(e:FullScreenEvent) {
			...
			if (_isFullScreen) {
				_output.appendText("mouseHide\n");
				Mouse.hide();
			} else {
				_output.appendText("mouseShow\n");
				Mouse.show();
			}
			_output.appendText("fullscreen resizeHandler\n");   
			resizeHandler(null);
			/*
			if (!e.fullScreen) {
				_controlBar.visible = _alwaysShowControls;
		...
		function repositionVideo():void {
			...
			_output.appendText("Screen dimension: "+_stageWidth+" x "+_stageHeight+"\n");
			_output.appendText("Native dimension: "+_nativeVideoWidth+" x "+_nativeVideoHeight+"\n"+"\n");
			...
			if (nativeRatio > stageRatio) {
				_mediaElement.setSize(_stageWidth, _stageWidth / nativeRatio);					
				_video.y = _stageHeight/2 - (_stageWidth / nativeRatio)/2;
				_video.x = 0;
			} else if (stageRatio > nativeRatio) {
				_mediaElement.setSize( _stageHeight * nativeRatio, _stageHeight);
				_video.x = _stageWidth/2 - (_stageHeight * nativeRatio)/2;
				_video.y = 0;
			} else if (stageRatio == nativeRatio) {
				_video.x = 0;
				_video.y = 0;
